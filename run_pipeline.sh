...
# ---------------- modules/steps to run ----------------
# Entries are either:
#   "module <python.module.path>"
#   "run <python.module.path> <args...>"
MODULES=(
  "module src.runners.Preload"                      # optional
  "module src.runners.DataProcessing"               # optional
  "module src.controllers.RunDataExplorer"
  "module src.controllers.AMR.scripts.run_comprehensive_analysis"
  "module src.runners.CompleteTemporalAnalysis"
  "module src.runners.SummaryStatistics"

  "run src.controllers.Run \
    --data ./datasets/structured/dataset_parquet \
    --out ./output \
    --min-test-count 2000 \
    --min-res-count 5 \
    --min-test-rate 0.1 \
...
for entry in "${MODULES[@]}"; do
  ...
  if [[ "$kind" == "module" ]]; then
    if ! runpymod $rest; then
      echo "$entry failed. Aborting."
      exit 1
    fi
  elif [[ "$kind" == "run" ]]; then
    # rest starts with "<module> <args...>"
    if ! runpymod $rest; then
      echo "$entry failed. Aborting."
      exit 1
    fi
  else
    echo "Unknown step type: '$kind' in entry: $entry"
    exit 2
  fi
done
