...
from src.utils.helpers import pick_abx_cols

# Ensure altair is configured
...
# Placeholder functions used in load_data (must be defined or imported)
def prepare_feature_inputs(df, class_data):
    # Simplified placeholder for feature preparation logic
    antibiotics = [c for c in df.columns if ' - ' in c]
    class_map = {}
    who_class_map = {}
    return antibiotics, class_map, who_class_map

def compute_row_features(row, antibiotics, class_map, who_class_map):
    # Simplified placeholder for row-wise feature calculation
    return pd.Series([0, 0, 0], index=['NumDrugClassesTested', 'IsCriticalDrugTested', 'BackupDrugsTested'])
...
def run_full_descriptor(df, name, out_root): pass # Placeholder
...
class DataProcessing:
    def __init__(
        ...
        # RE-INSTATED FLEXIBLE PARAMETERS
        outcome_scheme: str = "raw_RSI",   # "RI_vs_S" | "R_vs_SI" | "ordinal_R2I1S0" | "raw_RSI"
        outcome_dtype: str = "Int8"        # pandas nullable integer (or 'string' for raw_RSI)
    ):
        """
        ...
        self.outcome_scheme = outcome_scheme # Stored for use in preprocess_data
        self.outcome_dtype = outcome_dtype   # Stored for use in preprocess_data
    ...
    # ---------- R/I/S normalization & outcome encoding (REFINED) ----------

    @staticmethod
    def _norm_result_token(x):
        """
        Normalizes input values (0, 1, R, S, I, etc.) to canonical 'R', 'S', 'I', or NaN.
        """
    ...
    @staticmethod
    def _encode_outcome(tokens: pd.Series, scheme: str, dtype: str) -> pd.Series:
        """
        Encodes normalized R/I/S tokens into the specified numerical scheme or returns raw strings.
        """
        # Scheme 4: Raw Strings (R/I/S) - For reporting/non-ML purposes
        if scheme == "raw_RSI":
            # Returns the normalized R/I/S tokens directly, using nullable string dtype
            return tokens.astype("string").mask(tokens.isna(), pd.NA) 

        # Scheme 1: Resistant/Intermediate vs Susceptible
        if scheme == "RI_vs_S":
        ...
        # Scheme 2: Resistant vs Susceptible/Intermediate - For binary ML focusing on R
        if scheme == "R_vs_SI":
            out = pd.Series(pd.NA, index=tokens.index, dtype=dtype)
            out = out.mask(tokens.isna(), pd.NA)
            # 1 (Resistant) when token is R
            out = out.where(~(tokens == "R"), 1)
            # 0 (Non-Resistant) when token is S or I
            out = out.where(~tokens.isin(["S", "I"]), 0) 
            return out.astype(dtype)

        # Scheme 3: Ordinal (R=2, I=1, S=0) - For ordinal regression/ABN
    ...
    def preprocess_data(self):
        ...
        # Determine the appropriate outcome dtype based on the scheme
        outcome_dtype = self.outcome_dtype
        if self.outcome_scheme == "raw_RSI":
            outcome_dtype = "string" 
            print(f"[INFO] Outcome DType set to '{outcome_dtype}' for raw R/I/S tokens.")
        else:
             print(f"[INFO] Outcome DType set to '{outcome_dtype}' for numerical encoding.")
        ...
        outcome_cols = {}
        for col in self.antibiotic_columns:
            tokens = self.data[col].apply(self._norm_result_token)  # returns R, I, S, or NaN
            
            # Use the flexible encoder based on self.outcome_scheme
            outcome_cols[f"{col}_Outcome"] = self._encode_outcome(
                tokens, scheme=self.outcome_scheme, dtype=outcome_dtype
            )
    ...
    def enrich_data(self, df: pd.DataFrame) -> pd.DataFrame:
        # Assuming PatientAge column needs to be extracted from another column, 
        # or it is missing in the full code structure but intended to be used.
        # Placeholder to ensure it runs if PatientAge is not present in original data
        if "PatientAge" not in df.columns:
            df["PatientAge"] = df["AgeRange"].str.extract(r"(\d+)").astype(float) # Use AgeRange as fallback/example
...
def run_data_pipeline():
    # Example using numerical encoding for ABN/ML
    # data_loader_numerical = DataProcessing(
    #     file_path="./datasets/interim/CompleteData_en/dataset_parquet",  # parquet directory or file
    #     output_path="./datasets/output/numerical",
    #     antibiotics_class_file="./datasets/antibiotic_classification_complete.csv",
    #     outcome_scheme="ordinal_R2I1S0"  # R=2, I=1, S=0; NaN preserved as <NA> (numerical)
    # )
    # Example using raw strings for reporting/ARM
    data_loader_raw = DataProcessing(
        file_path="./datasets/interim/CompleteData_en/dataset_parquet",  # parquet directory or file
        output_path="./datasets/output/raw_strings",
        antibiotics_class_file="./datasets/antibiotic_classification_complete.csv",
        outcome_scheme="raw_RSI",  # R/I/S strings; NaN preserved as <NA>
        outcome_dtype="string" # Type hint for pandas nullable string
    )
    ...
    data_loader_raw.generateJson(data_loader_raw.data) # Use one of the loaders' data

if __name__ == "__main__":
    run_data_pipeline()
    print("Code updated successfully with flexible outcome encoding.")
